# Getting the current AWS account ID
...
# S3 bucket conversation memory
resource "aws_s3_bucket" "memory-bucket" {
  bucket = "${local.name_prefix}-memory-${data.aws_caller_identity.current.account_id}"
  tags   = local.common_tags
}

resource "aws_s3_bucket_public_access_block" "memory-bucket" {
  bucket = aws_s3_bucket.memory-bucket.id
...
resource "aws_s3_bucket_ownership_controls" "memory-bucket" {
    bucket = aws_s3_bucket.memory-bucket.id
...
# S3 bucket for static frontend website
resource "aws_s3_bucket" "frontend-bucket" {
  bucket = "${local.name_prefix}-frontend-${data.aws_caller_identity.current.account_id}"
  tags   = local.common_tags
}

resource "aws_s3_bucket_public_access_block" "frontend-bucket" {
...
resource "aws_s3_bucket_policy" "frontend-bucket" {
  bucket = aws_s3_bucket.frontend-bucket.id
  ...
  depends_on = [aws_s3_bucket_public_access_block.frontend-bucket]
}

# IAM roles for Lambda running the chatbot
...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      ...
      S3_BUCKET        = aws_s3_bucket.memory-bucket.id
      USE_S3           = "true"
      BEDROCK_MODEL_ID = var.bedrock_model_id
    }
  }

  # Lambda waits for the distribution to exist
...
resource "aws_cloudfront_distribution" "main" {
  ...
  origin {
    domain_name = aws_s3_bucket_website_configuration.frontend-bucket.website_endpoint
